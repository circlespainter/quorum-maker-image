...
echo $CYAN"Building Tessera, "$tessera_repository" (commit "$tessera_commit")..."$COLOR_END

rm -rf tessera

docker run -ti --rm -v ${HOME}:/root -v $(pwd):/git alpine/git clone $tessera_repository
docker run -ti --rm -v ${HOME}:/root -v $(pwd):/git -w /git/tessera alpine/git checkout $tessera_commit # tessera-$tessera_version
...
ln -s tessera-dist/tessera-app tessera/tessera-app
...
echo $CYAN"Building Tessera, "$tessera_repository" (commit "$tessera_commit") Completed."$COLOR_END
